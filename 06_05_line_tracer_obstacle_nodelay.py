...
def execute():
    ...
    while sim.getSimulationState() != sim.simulation_stopped:
        ...
        if state == "FOLLOWING":
            ...
            # Changed turning direction to RIGHT instead of LEFT
            t_start = sim.getSimulationTime()
            while sim.getSimulationTime() - t_start < 0.8:
                print("AVOIDANCE: turning right")
                # Changed motor values to turn right (positive left, negative right)
                ...
                if found_line:
                    ...
                    state = "SEARCH"
                    # Adjust search direction to match the new avoidance pattern
            ...
            while sim.getSimulationTime() - search_start_time < search_duration:
                ...
                # Changed search direction to be consistent with right-side avoidance
            ...
            if not(fimgR and fimgM and fimgL):
                ...
                state=3
            while True:
                # Modified alignment to be consistent with right-side obstacle avoidance
                sim.setJointTargetVelocity(motors["left"], 0.2)
                sim.setJointTargetVelocity(motors["right"], 0.0)
                imgL, imgM, imgR = sensor_ir(sensors["left"]), sensor_ir(sensors["mid"]), sensor_ir(sensors["right"])
                print("ALIGNMENT:",imgL , imgM ,imgR, state)
                
                if not(imgL and imgM) and (state==0):
                    break
                elif ( (imgR and imgM) and not(imgL) )  and (state==1):
                    break
                elif not(imgR and imgM) and (state==2):
                    break
